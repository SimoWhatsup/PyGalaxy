...
class spectralAnalysis(object):
    def __init__(self, obs, spectralConf):
        ...
        for i in xrange(7, obs.nx - 9):  # min:7 #max:1014
            for j in xrange(7, obs.ny - 9):  # 7
                ...
                if printing:
                    ...
                    self.logger.info(
                        "Suspected HISA value_array: %s" % residual[residual < sigma_pos * self.HISA_F_SPECTRAL])
                ...
                while counter < cnt1:
                    ...
                    if (smoothed[kmin[0][0]] > 2. * self.HISA_F_SPECTRAL * sigma_pos and unabsorbed[
                        kmin[0][0]] > self.TEMP_CRITICAL):
                        ...
                        if printing:
                            ...
                            self.logger.info(
                                "- (amp, mu, sigma)_narrow = (%s, %s, %s)" % (params[0], params[1], params[2]))
                            self.logger.info(
                                "- (amp, mu, sigma)_broad  = (%s, %s, %s)\n" % (params[0], params[1], params[3]))
                        ...
                        bestfit_paramsN = [float(paramsN['amp'].value), float(paramsN['mu'].value),
                                           float(paramsN['sigma'].value)]
                        bestfit_paramsB = [float(paramsB['amp'].value), float(paramsB['mu'].value),
                                           float(paramsB['sigma'].value)]
                        ...
                        condition = min([bestfit_paramsN[0] / max([sigma_fitN, sigma_pos]),
                                         bestfit_paramsB[0] / max([sigma_fitB, sigma_pos])])
        ...
        file = '/lustre/fs4/group/that/sf/Survey/CGPS/HISA/CGPS_' + commonConf['mosaic'] + '_spectral_search_pass' + \
               commonConf['spectral_iteration'] + '.fits'
