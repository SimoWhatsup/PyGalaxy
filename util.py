...
def correct_continuum2((T, vec)):
	...
	if mosaic == 'G348.0':
		...
		z3a = [5, 52, 68, 75, 88, 110, 148, 159, 159, 162, 162, 178, 178, 181, 181, 198, 217, 221, 221, 234, 234, 241,
			   244, 268, 268, 313]
...
def get_ampHISA(Tpeak, Tc, Tu, dy, dv, r, sigma, utilsConf):
	...
	if debug:
		print("> deltaT = %f, Tc = %f, Tu = %f" % (deltaT, Tc, Tu))
	...
	if debug:
		print("- A = %f, B = %f" % (A, B))
	if debug:
		print("- partau = %f, partspin = %f" % (par_tau, par_tspin))
	...
	for i in xrange(7, nx - 9):  # 7
		for j in xrange(7, ny - 9):
			...
			while (counter < cnt1):
				...
				if (smoothed[ikmin[0][0]] > 2. * HISA_F_SPECTRAL * sigma_pos and unabsorbed[
					ikmin[0][0]] > TEMP_CRITICAL):
					...
					condition = min([bestfit_parN[0] / max([sigma_fitN, sigma_pos]),
									 bestfit_parB[0] / max([sigma_fitB, sigma_pos])])
...
def plotFunc(x, func, lbl=None, position=None):
	...
	if n > 8:
		...
		exit(0)
	if lbl != 'no label':
		if position is None: position = 'upper left'
...
def getPath2(surveyLogger, key=opt_list, mode="DESY"):
	survey = key[0]
	spec = key[1]
	flag = key[2]
...
def writeConfig(surveyLogger, surveyDictionary, mosaicDictionary={}, utilsDictionary={}, spectralDictionary={},
				spatialDictionary={}):
	"""
