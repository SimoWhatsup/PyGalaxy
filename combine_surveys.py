...
class combineSurveys(object):
    def __init__(self, surveylist, mosaiclist, species, res):
        ...
        if nsurveys == 0:
            ...
            file1 = path1 + 'skymap_hi_unabsorbed_' + survey.lower() + '_rbands_r9_res' + str(
                res) + '_' + mosaic + '.fits'
            ...
            for i, srv in enumerate(surveylist):
                ...
                file = path + 'skymap_' + self.species.lower() + '_' + srv.lower() + '_rbands_r9_res' + str(res) + '_' + \
                       mosaiclist[i] + '.fits'
                f = pyfits.open(file)
                key[i] = f[0].header
                sky[i] = f[0].data
        ...
        if nsurveys == 1:
            ...
            skymap_name = path + 'skymap_hi_unabsorbed+hisa_' + survey.lower() + '_rbands_r9_res' + str(
                res) + '_' + mosaic + '.fits'
        else:
            srv_name = '_'.join(surveylist).lower()
            path = '/afs/ifh.de/group/that/work-sf/survey/batch/survey_skymaps/hi/'
            skymap_name = path + 'skymap_' + self.species.lower() + '_' + srv_name + '_rbands_r9_res' + str(
                res) + '.fits'
