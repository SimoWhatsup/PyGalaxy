...
class genericMosaic(object):
    def __init__(self, obs, mosaicConf):
        ...
        if self.survey == 'Galprop':
            ...
            self.rband_array = self.msc_ref_rband + self.msc_del_rband * (
            arange(self.msc_rband) + 1. - self.msc_ind_rband)
        ...
        if self.survey == 'LAB' or self.survey == 'SGPS':
            ...
            newheader.update(key="crval3", value=obs.vel_array[min(v1_px, v2_px) - 1] * 1e3,
                             comment="Velocity of reference pixel")
            ...
            newheader.update(key="object", value=self.survey + " Mosaic " + self.mosaic,
                             comment=self.survey + " Mosaic")
            if self.survey == 'LAB':
